...
Shader "azure[Sky]/v1_5/Animated Cloud"
{
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			float4 frag(v2f IN) : SV_Target
			{
			   ...
			   float4 C1 = tex2D(Cloud1, uv);
			   float4 C2 = tex2D(Cloud2, uv);
			   float4 Cloud  = lerp(C1, C2, cloudLerp);
			   ...
			   inScatter *= (Cloud.rgb * dayLuminance) * sunsetColor.rgb;
			   ...
			   // Night Sky //
			   ///////////////
			   float  topCloud   = saturate(Cloud.r * nightTopCloud * (viewDir.y-0.4));
			   float3 nightSky   = (pow( 1.0-fex2, 3.0) * nightIntensity) * (1-IN.Fade.x);												// Defaut night sky color
			          nightSky  *= (Cloud.rgb * nightLuminance);
			          nightSky  += topCloud;
			   ...
			   float3 moonLight  = saturate( (moonLightColor.rgb * moonLightIntensity) * pow(dot(viewDir, moonDir),10.0)  * IN.Fade.z );// Moonlight.
			   ...
			   //////////////////////
			   // Sky finalization //
			   float3 finalSky = inScatter + (nightSky + moonColor.rgb + moonLight + stars);
