...
Shader "azure[Sky]/Extra/v2.0/Fog Scattering"
{
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			uniform float4  _sunsetGlobalColor;
			uniform float4  _noonGlobalColor;
			uniform float4  _nightGlobalColor;
...
//			   zenith = (acos(max(0, dot(float3(0,1,0), sunDir))));
			   ...
			   blueObscurance = lerp(nightBlueObscurance, dayBlueObscurance, IN.Fade.x);
			   ...
			   //This affects the entire scene
			   float3 GlobalColor  =  lerp(_sunsetGlobalColor.rgb,_noonGlobalColor.rgb,saturate(sunDir.y));
			          GlobalColor  =  lerp(_nightGlobalColor.rgb,GlobalColor, saturate(sunDir.y+0.75));
			   
			   
			   float3 normalFogCol =  lerp(normalFog_sunsetColor.rgb,normalFog_noonColor.rgb,saturate(sunDir.y));
			          normalFogCol =  lerp(normalFog_nightColor.rgb ,normalFogCol, IN.Fade.x);
			   float3 normalFog    =  lerp(screen,normalFogCol, Mask);
			          normalFog    =  lerp(screen,normalFog, pow(saturate(depth * normalFogDistance),linearFog));
			          normalFog    =  pow(normalFog,colorCorrection);
			   
			   
			   float3 inScatteringFog =    lerp(screen,finalSky, Mask);                  						                             // Creating the fog color.
			          inScatteringFog =    lerp(screen, inScatteringFog * GlobalColor, pow(saturate(depth * inScatterFogDistance),linearFog)); // Mixing the fog with the scene, according to the depth.
			   
			   float3 finalFog = lerp(normalFog, inScatteringFog, pow(saturate(depth * mixFogDistance),linearFog));
